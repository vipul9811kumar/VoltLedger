...
import type { ApiTrace, OriginationHistoryResponse } from '@/lib/lender-portal/voltledger-client';
...
export default function NewApplicationPage() {
  ...
  const [originationHistory, setOriginationHistory] = useState<ApiTrace<OriginationHistoryResponse> | undefined>(undefined);
  ...
  async function handleLookup() {
    ...
    setOriginationHistory(undefined);
    try {
      const res = await fetch(`/api/lender-portal/lookup-battery?serial=${encodeURIComponent(batterySerial)}`);
      const { battery, originationHistory: history } = await res.json();
      if (!battery.ok) {
        setLookupError(battery.response?.body?.error ?? `Lookup failed (${battery.response?.status})`);
      } else {
        setLookupPreview(battery.response.body);
        setOriginationHistory(history);
      }
    } catch (e) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setResult(null);
    try {
      const res = await fetch('/api/lender-portal/underwrite', {
        method: 'POST',
    ...
    <div className="space-y-8">
      ...
      <form onSubmit={handleSubmit} className="card space-y-4 max-w-2xl">
        ...
        <div>
          ...
          {originationHistory?.ok && originationHistory.response.body.audits.length > 0 ? (
            <p className="mt-2 text-xs text-yellow-400">
              ⚠ Previously financed {originationHistory.response.body.audits.length}×
              — most recent {new Date(originationHistory.response.body.audits[0].checkedAt).toLocaleDateString()}
              {' '}(grade {originationHistory.response.body.audits[0].riskGradeAtOrigination ?? '—'}).
