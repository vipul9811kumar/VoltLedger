...
export interface OriginationAuditSummary {
  id: string;
  checkedAt: string;
  passportTierAccessed: string | null;
  passportVerifiedAtCheck: boolean;
  sohSource: string;
  sohUsedPct: number | null;
  compositeScoreAtOrigination: number | null;
  riskGradeAtOrigination: RiskGrade | null;
  attestationVersion: string;
}

export interface OriginationHistoryResponse {
  batteryId: string;
  serial: string;
  audits: OriginationAuditSummary[];
}

export interface SecondLifeResponse {
  batteryId: string;
  serialNumber: string;
  assessedAt: string;
  currentSoH: number;
...
export function getOriginationHistory(serial: string) {
  return call<OriginationHistoryResponse>('GET', `/v1/origination/battery/${encodeURIComponent(serial)}`);
}
...
export function getSecondLife(serial: string) {
  return call<SecondLifeResponse>('GET', `/v1/batteries/${encodeURIComponent(serial)}/second-life`);
}
