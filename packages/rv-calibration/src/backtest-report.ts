...
export function renderBacktestReport(result: BacktestResult): string {
  const { rows } = result;

  const primaryRows = rows
    .map((r) => {
      const modeledChange = r.modeledPctChangeFromPrev == null ? '—' : `${r.modeledPctChangeFromPrev > 0 ? '+' : ''}${r.modeledPctChangeFromPrev}%`;
      const realChange = r.realEvIndexPctMoM == null ? '—' : `${r.realEvIndexPctMoM > 0 ? '+' : ''}${r.realEvIndexPctMoM}%`;
      const realYoY = r.realEvIndexPctYoY == null ? '—' : `${r.realEvIndexPctYoY > 0 ? '+' : ''}${r.realEvIndexPctYoY}%`;
      return `| ${r.releaseLabel} | ${r.modeledIndexLevel} | ${modeledChange} | ${realChange} | ${realYoY} | ${r.directionAgreement} |`;
    })
    .join('\n');

  const comparable = rows.filter((r) => r.directionAgreement !== 'N/A');
  const agreeCount = comparable.filter((r) => r.directionAgreement === 'AGREE').length;

  return `# RV Market Back-Test (WS-C)
...
**Direction agreement: ${agreeCount}/${comparable.length} comparable months.**
